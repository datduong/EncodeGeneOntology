...
filter_positive = function(fin){
  ...
  negative = negative[1:3000, ]
  positive2 = positive2[1:3000, ]
...
'Elmo/cosine.1024Jan26/go_analysis_all.degree.txt',
...
# 'BERT/cosine768Linear768Layer11+12nnParamOpW2/go_analysis_all.degree.txt',
...
plot_name = c('AIC','BiLSTM','ELMO', 'BERT As Service', 'BERT Mean Layer 12', 'BERT CLS Token', 'GCN', 'BERT Encode GO Name', 'Onto2vec') # 'BERT11+12'
# plot_name = c('AIC','BiLSTM', 'BERT1', 'BERT2', 'BERT3', 'BERT4', 'GCN', 'Onto2vec')


dfmain = read.table(file_list[1],header=T,sep="\t",row.names=NULL)
dfmain = subset (dfmain, dfmain$ic1>2 & dfmain$ic2>2)
# dfmain = subset(dfmain , dfmain$degree1<100 & dfmain$degree2<100) # 480
...
label_name = list()
label_name[['ic']] = 'Min IC of GO pair'
...
for (i in seq(1,length(plot_name))) {
  ...
  if (choice == 'ic'){
    xlabel = levels ( cut_width(df$MinIc,1) ) ## use 5 for MaxDegree
    plot1 = ggplot(df) +
    geom_boxplot(aes(x=cut_width(MinIc,1),y=score,fill=label), outlier.size=0.5) + # group = cut_width(carat, 0.5) # MinIc
    geom_smooth(method = "lm", se=FALSE, aes(x=cut_width(MinIc,1),y=score,group=label, color=label) ) +
  ...
  if (choice == 'degree') {
    ...
    scale_color_manual( values=c("black","black"), name = "", labels = "")
  }
  ...
  ggtitle(plot_name[i]) + ylab("cosine sim. score") +
  ...
  if ( ! plot_name[i] %in% c('AIC','ELMO','BERT As Service','BERT Encode GO Name','Onto2vec') ) {
...
pp = ggarrange(allplot[[1]],allplot[[2]],allplot[[3]],allplot[[4]],
          ...
          allplot[[8]], # COMMENT swap GCN head
          allplot[[9]],
          # allplot[[10]],
          common.legend = TRUE, ncol=3, nrow=3)

pp
ggsave( paste0(choice,".pdf"), pp , device='pdf',width=10,height=7,units='in' )
