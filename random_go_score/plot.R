...
file_list = c('AIC/Human_go_analysis_all.AIC.degree.txt', 'GCN/cosine.300/go_analysis_all.GCN.degree.txt', 'BERT/cosine.768.reduce300ClsVec/go_analysis_all.BERT.degree.txt', 'BiLSTM/cosine.bilstm.300Vec/go_analysis_all.BiLSTM.degree.txt' )
plot_name = c('AIC','GCN','BERT','BiLSTM')
...
for (i in seq(1,4)) {
  ...
  plot1 = ggplot(df) +
  geom_boxplot(aes(x=cut_width(MinIc,.5),y=score,fill=label), outlier.size=0.5) + # group = cut_width(carat, 0.5)
  scale_x_discrete( name="Range of min IC of GO pair", labels=xlabel ) + # , breaks=seq(0, 420, 10), labels=seq(0,420,10)
  ...
  ggsave(paste0(plot_name[i],'_GO_pairs_ic.PNG'),device='png',width=12,height=3,units='in')
