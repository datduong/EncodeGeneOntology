...
CUDA_VISIBLE_DEVICES=4 python3 $server/GOmultitask/GCN/encoder/do_model.py --vocab_list $vocab_list --w2v_emb $w2v_emb --lr 0.001 --main_dir $work_dir --qnli_dir $data_dir --batch_size_label_desc 128 --batch_size_label 20 --batch_size_bert 8 --result_folder $result_folder --epoch 100 --use_cuda --metric_option $metric_option --nonlinear_gcnn $nonlinear_gcnn --def_emb_dim $def_emb_dim --fix_word_emb --word_mode $word_mode --bilstm_dim 1024 > $result_folder/train.log
...
server='/local/datdb'
work_dir=$server/'goAndGeneAnnotationMar2018'
data_dir=$server/'goAndGeneAnnotationMar2018/entailment_data/AicScore/go_bert_cls'
...
CUDA_VISIBLE_DEVICES=3 python3 $server/GOmultitask/GCN/encoder/do_model.py --vocab_list $vocab_list --w2v_emb $w2v_emb --lr 0.001 --main_dir $work_dir --qnli_dir $data_dir --batch_size_label_desc 64 --batch_size_label 32 --result_folder $result_folder --epoch 100 --use_cuda --metric_option $metric_option --nonlinear_gcnn $nonlinear_gcnn --def_emb_dim $def_emb_dim --fix_word_emb --word_mode $word_mode > $result_folder/train.log
