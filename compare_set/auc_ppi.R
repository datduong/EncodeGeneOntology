...
path = '/u/flashscratch/d/datduong/YeastPPI3ontology/'
setwd(path)
# 'GCN.cosine.768' , 'cosine.AveWordClsSep768.Linear768.Layer12' , 'cosine.bilstm768' , 'cosineCls768Linear768'
model_type = c( 'cosine768Linear768Layer11+12nnParamOpW2' )
...
for (ontology in c("all3") ) { ## "BP","CC","MF",
	...
	result = read.table(paste0('scoreYeastPPI',ontology,'wSimDef.txt'),header=F) # wSimDef
	...
	for (next_data in model_type) {
		print (next_data)
		next_data_file = read.table(paste0('qnliFormatData17/',next_data,'/GeneScore.txt'),header=F,sep="\t")
