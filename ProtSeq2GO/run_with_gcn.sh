...
for fold in 1 ; do 
  ...
  for ontology in bp mf cc ; do # bp mf cc ;
    ...
    model_choice='DeepGOFlatSeqProtHwayGo'
    result_folder=$data_dir/'FlatSeqProtHwayGCN768.'$ontology
    mkdir $result_folder
    label_counter_dict=$deepgo_dir/'CountGoInTrain-'$ontology'.pickle'
    ...
    CUDA_VISIBLE_DEVICES=5 python3 $server/GOmultitask/ProtSeq2GO/do_model_gcn.py --model_choice $model_choice --has_ppi_emb --prot_interact_vec_dim $prot_interact_vec_dim --ontology $ontology --do_kmer --gcnn_dim $gcnn_dim --def_emb_dim $def_emb_dim --go_vec_dim $def_emb_dim --prot_vec_dim 832 --optim_choice SGD --lr 0.001 --main_dir $work_dir --data_dir $data_dir --batch_size_label 32 --result_folder $result_folder --epoch 100 --use_cuda --metric_option $metric_option --nonlinear_gcnn $nonlinear_gcnn --go_enc_model_load $go_enc_model_load --label_subset_file $label_subset_file --fix_go_emb --label_counter_dict $label_counter_dict --prot2seq_model_load $prot2seq_model_load > $result_folder/train.log

    # --w2v_emb $w2v_emb RMSprop
