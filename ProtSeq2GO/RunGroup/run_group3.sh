...
for model_choice in DeepGOFlatSeqConcatGo ; do # 'DeepGOFlatSeqProtHwayGo'
  ...
  for lr in 0.0001 ; do
    for fold in 1 ; do
      for ontology in bp ; do # cc bp bp mf cc
        ...
        mkdir $result_folder
        result_folder=$data_dir/$model_choice'UniformGOVector/NotNormalizeSgd'$lr$sgd_lr #$add_name
        mkdir $result_folder
...
w2v_emb=$work_dir'/word_pubmed_intersect_GOdb_w2v_pretrained.pickle'
vocab_list=$work_dir/'word_pubmed_intersect_GOdb.txt' # all vocab to be used, can be BERT or something, so we don't hardcode
precomputed_vector='/local/datdb/deepgo/data/cosine.Cls768.Linear768/label_vector.pickle'  ####
work_dir=$server/'deepgo/data' ##!! redefine ... should probably not need this if we don't hardcode extra needed stuffs.
batch_size_aa_go=32 #24 # 32
weight_decay=0
sgd_lr=0.001
  ...
  for lr in 0.0005 ; do
    for fold in 1 ; do
      for ontology in mf cc bp ; do # cc bp bp mf cc
        ...
        ##!!##!!##!!##!!
        result_folder=$data_dir/$model_choice'BertCLS12' #$add_name
        mkdir $result_folder
        result_folder=$data_dir/$model_choice'BertCLS12/NotNormalize' #$add_name
        mkdir $result_folder
