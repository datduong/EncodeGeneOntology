...
import GCN.encoder.data_loader as LabelDescDataLoaderClass ## COMMENT used to load data about one single label (just one definitions) ... too lazy to port out.
...
if args.tree : #### create maxpool just like how deepgo does it. ??, then we need to load the entire BP/CC/MF
  ...
  ##!! Code meant to do consistency-correction during training ... not possible ... too slow, and unstable ##!! ####
...
GO_loader_to_get_GO_vec, GO_name_to_get_GO_vec = LabelDescDataLoaderClass.LakeLabelDescLoader (LabelSamples,args.batch_size_pretrain_bert,fp16=False) ## if we fix encoder, then we don't have to worry about batch size, should be able to handle 32 or even 64
...
if args.fix_go_emb and (args.go_vec_dim > 0):
  ...
  with torch.no_grad():
    if args.precomputed_vector is None:
      print ('\n\n args.precomputed_vector is none, so we write out label into @go_emb variable. \n\n')
      go_emb = GOEncoder.write_label_vector(GO_loader_to_get_GO_vec,fout_name=None,label_name=None) ## go_emb is num_go x dim, @label_name is only needed if @fout_name is used
    else:
      go_emb = GOEncoder.forward(GO_name_to_get_GO_vec) # @GO_name_to_get_GO_vec is just some name array, as long as it matches @label_to_test

    print ('dim of go vectors {}'.format(go_emb.shape))
    # https://pytorch.org/docs/stable/nn.functional.html#normalize
    # other_params['go_emb'] = F.normalize( torch.FloatTensor(go_emb), dim=1 ).cuda() ## too dangerous to put it outside
    other_params['go_emb'] = torch.FloatTensor (go_emb [ other_params['label_to_test_index'] ]).cuda() ## we can't run on all GO labels (too many 40k total)
