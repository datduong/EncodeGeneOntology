...
precomputed_vector='/local/datdb/Onto2Vec/GOVectorData/2016DeepGOData/onto2vec2016dim768.pickle'
  ...
  for fold in 1 ; do 
  for ontology in bp mf cc ; do # cc bp bp mf cc
    ...
    result_folder=$data_dir/'FlatSeqProtHwayOnto2vec.'$def_emb_dim.$ontology
    ...
    CUDA_VISIBLE_DEVICES=5 python3 $server/GOmultitask/ProtSeq2GO/do_model_precompvec.py --vocab_list $vocab_list --w2v_emb $w2v_emb --model_choice $model_choice --has_ppi_emb --prot_interact_vec_dim $prot_interact_vec_dim --ontology $ontology --do_kmer --go_vec_dim $def_emb_dim --prot_vec_dim 832 --optim_choice RMSprop --lr 0.001 --main_dir $work_dir --data_dir $data_dir --batch_size_label 32 --result_folder $result_folder --epoch 50 --use_cuda --metric_option $metric_option --vector_file $precomputed_vector --label_subset_file $label_subset_file --label_in_ontology $label_in_ontology --fix_go_emb --def_emb_dim $def_emb_dim --reduce_cls_vec > $result_folder/train.log # --prot2seq_model_load $prot2seq_model_load
